  ...
  async create(req = request, res = response) {
    ...
    try {
      ...
      await t.commit();
        ...
        // Obtener el rol del usuario que creó la cita
        const usuarioCreador = await Usuario.findByPk(req.user.userId, {
            include: [{ model: Rol, as: "rol" }]
        });
        const rolCreador = usuarioCreador?.rol?.nombre;

        // Emitir notificaciones según el tipo de usuario que crea
        const io = req.app.get("io");
        
        if (rolCreador === 'administrador') {
            // ADMIN crea → Notificar a BARBERO y CLIENTE
            if (citaCompleta.barbero?.usuario) {
                io.to(`barbero_${citaCompleta.barbero.usuario.id}`).emit("nueva_cita", {
                    tipo: "cita_creada",
                    cita: citaCompleta,
                    mensaje: `Nueva cita asignada para el ${new Date(citaCompleta.fecha).toLocaleDateString()} a las ${citaCompleta.hora.substring(0, 5)}`
                });
            }

            if (citaCompleta.cliente) {
                // Buscar usuario del cliente
                const usuarioCliente = await Usuario.findOne({
                    where: { id: citaCompleta.cliente.usuarioID }
                });
                if (usuarioCliente) {
                    io.to(`cliente_${usuarioCliente.id}`).emit("nueva_cita", {
                        tipo: "cita_creada",
                        cita: citaCompleta,
                        mensaje: `Tienes una nueva cita para el ${new Date(citaCompleta.fecha).toLocaleDateString()} a las ${citaCompleta.hora.substring(0, 5)}`
                    });
                }
            }
            ...
            io.to('administradores').emit("nueva_cita", {
                tipo: "cita_creada_por_barbero",
                cita: citaCompleta,
                mensaje: `El barbero ${usuarioCreador.nombre} creó una nueva cita`
            });
            ...
            if (citaCompleta.cliente) {
                ...
                if (usuarioCliente) {
                    io.to(`cliente_${usuarioCliente.id}`).emit("nueva_cita", {
                        tipo: "cita_creada",
                        cita: citaCompleta,
                        mensaje: `El barbero te asignó una cita para el ${new Date(citaCompleta.fecha).toLocaleDateString()} a las ${citaCompleta.hora.substring(0, 5)}`
                    });
                }
            }
            ...
            if (citaCompleta.barbero?.usuario) {
                io.to(`barbero_${citaCompleta.barbero.usuario.id}`).emit("nueva_cita", {
                    ...
                    cita: citaCompleta,
                    mensaje: `El cliente ${citaCompleta.cliente?.nombre || 'Cliente'} solicitó una cita`
                });
            }
